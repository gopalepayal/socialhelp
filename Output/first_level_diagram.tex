...
\node[entity] (Adviser) at (-7, -9) {Donors \& Organisations};
\node[entity] (Viewer) at (-7, -13.5) {System Logs / Monitors};

% Right External Entities (x = 7)
\node[entity] (Admin) at (7, -9) {System Administrator};

% Left Data Stores (x = -3.5)
\node[datastore] (D2) at (-3.5, -4.5) {Verifications DB};

% Right Data Stores (x = 3.5)
\node[datastore] (D1) at (3.5, -1) {Profiles DB};
\node[datastore] (D3) at (3.5, -7) {Donations DB};
\node[datastore] (D4) at (3.5, -9.5) {Requirements DB};
\node[datastore] (D5) at (3.5, -11) {Chat Logs DB};

% -------------------------------------------------------------------------
% Structural Lines mapped using explicit coordinate joints
% -------------------------------------------------------------------------

% LEFT BUS (Adviser/Donor Backbone)
\draw[-, thick] (-9.2, 0) -- (-9.2, -15);
\draw[-] (Adviser.west) -- (-9.2, -9);
\draw[->] (-9.2, 0) -- (P1.west);
\draw[->] (-9.2, -15) -- (P6.west);

% External strictly routed hook safely above the D2 Database down to P2
...
\caption{1st Level Data Flow Diagram for Social Help and Donation System}
\end{figure}
